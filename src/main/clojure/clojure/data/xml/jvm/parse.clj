...
(defn pull-seq
  ...
  ([^XMLStreamReader sreader include-node? ns-envs]
   (pull-seq sreader include-node? identity ns-envs))
  ([^XMLStreamReader sreader include-node? event-fn ns-envs]
   (lazy-seq
     (loop []
       (static-case
         (.next sreader)
         ...
         (if-let [text (and (include-node? :characters)
                            ...
                            (.getText sreader))]
           (cons (event-fn (->CharsEvent text))
                 (pull-seq sreader include-node? event-fn ns-envs))
           (recur))
         XMLStreamConstants/COMMENT
         (if (include-node? :comment)
           (cons (event-fn (->CommentEvent (.getText sreader)))
                 (pull-seq sreader include-node? event-fn ns-envs))
           (recur))
...
(defn attach-location-meta
  [^XMLStreamReader sreader event]
